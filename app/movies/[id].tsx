...
const MovieInfo = ({label, value}: MovieInfoProps) => (
  <View className='flex-col items-start justify-center mt-5'>
    <Text className='text-gray-400 font-normal text-sm mt-5'>
      {label}
    </Text>
  ...
  return (
    <View className='bg-primary flex-1'>
      <ScrollView contentContainerStyle={{paddingBottom: 80}}>
        <View>
          <Image
            source={{uri: `https://image.tmdb.org/t/p/w500${movie?.poster_path}`}} className='w-full h-[550px]' resizeMode="stretch"/>
        </View>
        <View className='flex-col items-start justify-center mt-5 px-5'>
          ...
          <View className="flex flex-row justify-between w-1/2 gap-14">
            <MovieInfo
              label="Budget"
      ...
      <TouchableOpacity className='absolute bottom-5 left-0 right-0 mx-5 bg-darkAccent rounded-lg py-3.5 flex flex-row items-center justify-center z-50' onPress={router.back}>
